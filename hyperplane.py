...
class Hyperplane(object):
    ...
    EITHER_DIM_OR_NORMAL_VECTOR_MUST_BE_PROVIDED = 'Either dimension of the hyperplane or normal vector must be provided'

    def __init__(self, dimension=None,normal_vector=None, constant_term=None):
        if not dimension and not normal_vector:
            raise Exception(self.EITHER_DIM_OR_NORMAL_VECTOR_MUST_BE_PROVIDED)
    ...
    def is_parallel_to(self,pll):
      n1 = self.normal_vector
      n2 = pll.normal_vector
      return n1.is_parallel_to(n2)

    def __eq__(self,pll):

        if self.normal_vector.is_zero():
            if not pll.normal_vector.is_zero():
                return False
            else:
                diff = self.constant_term - pll.constant_term
                return MyDecimal(diff).is_near_zero()
        elif pll.normal_vector.is_zero():
            return False

        if not self.is_parallel_to(pll):
            return False
